...
var API_KEY = 'AIzaSyAtUT5-7drpDhRoWrgm-tbQZ2X0y6cuosg';
var PUSH_SERVER_URL = 'https://physicalwebgradwerk.appspot.com/';
...
function subscribeDevice() {
  console.log('Try to subscribeDevice');
  // Disable the switch so it can't be changed while
  // we process permissions
  window.PushDemo.ui.setPushSwitchDisabled(true);

  navigator.serviceWorker.ready.then(function(){console.log('ready')},function(){console.log('notready')});
...
window.addEventListener('UIReady', function() {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/physicalwebdemo/service-worker.js?s=1', {
      scope: './physicalwebdemo/'
    })
    .then(initialiseState,failit);
  } else {
